#include "double_quotes.h"

Token DoubleQuotes::Parse(const std::string &command, size_t start_pos) const {
    std::string token = "";
    size_t pos = start_pos + 1;
    while (pos < command.length()) {
        if (command[pos] == '"') {
            if (IsEndOfCommand(command, pos))
                return Token(token, pos);
            else
                return Token(token, pos + 1);
        }

        size_t next = command.find('"', pos);

        if (next == std::string::npos)
            return Token(token, pos);

        size_t token_length = next - pos;
        token += command.substr(pos, token_length);

        pos = next;
    }

    return Token(token, pos);
}

bool DoubleQuotes::IsActivated(char c) const { return c == '"'; }

NodeType DoubleQuotes::GetNodeType() const { return NodeType::DOUBLE_QUOTES; }
